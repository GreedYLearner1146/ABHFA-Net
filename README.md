...
7) Run Hellinger_Bhatt_dist.py, which contains the Bhattacharyya coefficient computation codes. Take note that in the code, the square of the Hellinger distance is taken, but that can be considered as equivalent to the finding the 1 - Bhattarcharyya coefficient term, which in term could be used to extract the Bhattarcharyya coefficient term.
8) Now run ABHFANet main.py which comprised of the main backbone of the ABHFA-Net.
9) Run Sampler_and_Loader.py which provides the task sampler for the training and testing dataloader.
10) A series of functions is found inside a file labelled "demo-notebook" (placeholder name). They contained the helper functions to run the Bhattarcharyya-Hellinger Softmax (BHAS) loss function. The sequence in which they should be run is: Common_functions.py -> loss_and_miners_utils.py -> Module_With_Records.py -> Base_Reducers.py -> MeanReducer.py -> MultipleReducers_Do_Nothing_Reducers.py -> BaseDistances.py -> LpDistance.py -> ModulesWithRecordsandReducer.py -> Mixins.py -> BaseMetricLossFunction.py -> GenericPairLoss.py -> BhattHellingerLoss.py. The helper functions are mainly adapted from the pytorch metric learning library by Kevin Musgrave: https://github.com/KevinMusgrave/pytorch-metric-learning.
...
5) Run Attention.py, which contains the codes for the channel-spatial attention mechanism.
6) Run the ResNet-12 backbone, which is a modified ResNet12 with the channel-spatial attention imbued into it.
7) Run Hellinger_Bhatt_dist.py, which contains the Bhattacharyya coefficient computation codes. Again, take note that in the code, the square of the Hellinger distance is taken, but that can be considered as equivalent to the finding the 1 - Bhattarcharyya coefficient term, which in term could be used to extract the Bhattarcharyya coefficient term.
8) Now run ABHFANet main.py which comprised of the main backbone of the ABHFA-Net.
9) Run Sampler_and_Loader_miniImageNet.py which provides the task sampler for the training and testing dataloader.
10) Run eval_on_val.py, which entailed running the function "evaluate_on_one_task" and "evaluate" on the validation dataset.
11) A series of functions is found inside a file labelled "demo-notebook" (placeholder name). They contained the helper functions to run the Bhattarcharyya-Hellinger Softmax (BHAS) loss function. The sequence in which they should be run is: Common_functions.py -> loss_and_miners_utils.py -> Module_With_Records.py -> Base_Reducers.py -> MeanReducer.py -> MultipleReducers_Do_Nothing_Reducers.py -> BaseDistances.py -> LpDistance.py -> ModulesWithRecordsandReducer.py -> Mixins.py -> BaseMetricLossFunction.py -> GenericPairLoss.py -> BhattHellingerLoss.py. The helper functions are mainly adapted from the pytorch metric learning library by Kevin Musgrave: https://github.com/KevinMusgrave/pytorch-metric-learning.
12) Run training_fit_miniImageNet.py to begin training. The difference between training_fit_miniImageNet.py and training_fit.py is the different weight coefficient assigned to the categorical crossentropy and the Bhattarcharyya-Hellinger Softmax loss function (0.7 and 0.3, in contrast to the 1.0 and 0.5 in training_fit.py).
13) Run test_miniImageNet_loading.py to load the miniImageNet test images and dataloader.
14) Run test_sampler_and_loader_miniImageNet.py which provides the task sampler for the testing dataloader for the miniImageNet.
15) Finally, evaluate the trained model using Eval.py.
...
<img width="490" height="290" alt="Untitled-1" src="https://github.com/user-attachments/assets/edd919a5-78fa-4f80-957d-5981b9ad945c" /> \
...
The few-shot evaluation utilized are the 5-way-1-shot and 5-way-5-shot approach. The table below tabulated the relevant values obtained for the AIDER, CDD (aerial disaster image classification) and miniImageNet (benchmark) dataset when using the 5-way-1-shot and 5-way-5-shot approaches (in %):

| Method | 5-way-1-shot (%) | 5-way-5-shot (%) |
| ------ | ------| ------| 
|ATTBHFA-Net (AIDER)| **67.0 $\pm$ 1.7** | **76.9 $\pm$ 0.3** |
|ATTBHFA-Net (CDD)| **58.8 $\pm$ 1.0** | **62.6 $\pm$ 0.4** |
|ATTBHFA-Net (miniImageNet)| **73.4 $\pm$ 1.3** | **87.4 $\pm$ 0.7**  |
