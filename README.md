...
10) A series of functions is found inside a file labelled "demo-notebook" (placeholder name). They contained the helper functions to run the Bhattarcharyya-Hellinger Softmax (BHAS) loss function. The sequence in which they should be run is: Common_functions.py -> loss_and_miners_utils.py -> Module_With_Records.py -> Base_Reducers.py -> MeanReducer.py -> MultipleReducers_Do_Nothing_Reducers.py -> BaseDistances.py -> LpDistance.py -> ModulesWithRecordsandReducer.py -> Mixins.py -> BaseMetricLossFunction.py -> GenericPairLoss.py -> BhattHellingerLoss.py. The helper functions are mainly adapted from the pytorch metric learning library by Kevin Musgrave: https://github.com/KevinMusgrave/pytorch-metric-learning.
11) Run training_fit_miniImageNet.py to begin training. The difference between training_fit_miniImageNet.py and training_fit.py is the different weight coefficient assigned to the categorical crossentropy and the Bhattarcharyya-Hellinger Softmax loss function (0.7 and 0.3, in contrast to the 1.0 and 0.5 in training_fit.py).
12) Run test_miniImageNet_loading.py to load the miniImageNet test images and dataloader.
13) Run test_sampler_and_loader_miniImageNet.py which provides the task sampler for the testing dataloader for the miniImageNet.
14) Finally, evaluate the trained model using Eval.py.
